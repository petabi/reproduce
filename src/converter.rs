...
impl Converter {
    ...
    pub fn with_packet(l2_type: u32, matcher: Option<Matcher>) -> Self {
...
impl PacketConverter {
    fn new(l2_type: u32, matcher: Option<Matcher>) -> Self {
        Self { l2_type, matcher }
    }

    fn convert(&mut self, event_id: u64, input: &[u8], msg: &mut SizedForwardMode) -> Result<bool> {
        let (input, _) = if let Ok(r) = parse_pcap_packet(input) {
            r
        } else {
            return Err(
                io::Error::new(io::ErrorKind::InvalidData, "invalid captured packet").into(),
            );
        };
        if self.l2_type != 1 {
...
fn parse_pcap_packet(input: &[u8]) -> IResult<&[u8], &[u8]> {
    const PCAP_PACKET_HEADER_LEN: usize = 16;
    take(PCAP_PACKET_HEADER_LEN)(input)
}
...
mod tests {
    ...
    fn packet_converter_tcp() {
        ...
        let pkt1: Vec<u8> = vec![
            ...
            0x00, 0x39, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x08, 0x00, 0x45, 0x00, 0x00, 0x2B, 0x00, 0x01, 0x00, 0x00, 0x40, 0x06, 0x7C, 0xCA,
            0x7F, 0x00, 0x00, 0x01, 0x7F, 0x00, 0x00, 0x01, 0x00, 0x14, 0x00, 0x50, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x50, 0x02, 0x20, 0x00, 0xCD, 0x16, 0x00, 0x00,
            0x61, 0x62, 0x63,
        ];
        let pkt2: Vec<u8> = vec![
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x39, 0x00, 0x00,
            0x00, 0x39, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x08, 0x00, 0x45, 0x00, 0x00, 0x2B, 0x00, 0x01, 0x00, 0x00, 0x40, 0x06, 0x7C, 0xCA,
            0x7F, 0x00, 0x00, 0x01, 0x7F, 0x00, 0x00, 0x01, 0x31, 0x32, 0x61, 0x62, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x50, 0x02, 0x20, 0x00, 0xCD, 0x16, 0x00, 0x00,
            0x31, 0x32, 0x33,
        ];
        let exps = "abc\nxyz\n";
        let matcher = Matcher::from_read(exps.as_bytes()).expect("valid exps");
        let mut conv = PacketConverter::new(1, Some(matcher));
        let mut msg = SizedForwardMode::default();
        for _ in 0..MSG_LEN {
    ...
    fn packet_converter_vlan() {
        let pkt: Vec<u8> = vec![
            ,
            ...
            0x6B, 0x6C, 0x6D, 0x6E, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A,
            0x6B, 0x6C, 0x6D, 0x6E, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A,
            0x6B, 0x6C, 0x6D, 0x6E, 0x65, 0x65,
        ];
        let mut conv = PacketConverter::new(1, None);
        let mut msg = SizedForwardMode::default();
